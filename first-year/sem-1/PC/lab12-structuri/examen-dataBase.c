...
Sa se implementeze o baza de date care contine urmatoarele tipuri de produse:
 ...
 -baterie: voltaj
Fiecare produs va avea asociat un pret, stoc si un cod unic.
Operatiile vor fi:
 -introducere - introduce un nou produs, inclusiv codul sau. Daca codul exista 
 deja, se incrementeaza stocul, altfel se considera stoc = 1. 
 -listare - se listeaza doar produsele pe stoc
 -stergere - se da un cod si se decrementeaza stocul. Produsele cu stoc 0 raman
 in baza de date. Daca deja este 0, se emite mesaj de atentionare.
